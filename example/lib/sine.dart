...
void netTrainSine(TfannNetwork net, int rounds, double learningRate) {
  for (int i = 0; i < rounds; ++i) {
    ...
    net.train(TrainSetInputOutput.lists([x], [sin(x)]),
        ...
        maxErrClipAbove: 0.0001,
        skipIfErrBelow: 0.00001);
  }
}

void main() {
  {
    final sine_net = TfannNetwork.full([
      1,
      64,
      16,
      16,
    ...
    for (int i = 0; i < 1000; ++i) {
      x += 6.29 / 1000;
      print("x= $x ,   result: ${sine_net.feedForward(FVector.fromList([
                x
              ])).toList()} , real: ${sin(x)} error= ${sine_net.calculateMeanAbsoluteError([
